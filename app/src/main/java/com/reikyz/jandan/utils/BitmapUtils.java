...
public class BitmapUtils {
    ...
    /**
     * 使用canvas绘制文字drawable图片
     */
    public static Drawable textToDrawable(String s) {
        Paint paint = new Paint();
        paint.setTextSize(DentistyConvert.sp2px(15));
        Bitmap bitmap = Bitmap.createBitmap((int) paint.measureText(s) + 5, DentistyConvert.dp2px(21), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        int yPos = (int) ((canvas.getHeight() / 2) - ((paint.descent() + paint.ascent()) / 2) + 5);
        paint.setTextAlign(Paint.Align.LEFT);
        paint.setAntiAlias(true);
        paint.setColor(MyApp.getContext().getResources().getColor(R.color.blue));
        paint.setTypeface(Typeface.DEFAULT_BOLD);
        canvas.drawText(s, 5, yPos, paint);
        canvas.save();
        Drawable drawableright = new BitmapDrawable(bitmap);
        drawableright.setBounds(0, 0, canvas.getWidth(), canvas.getHeight());
